...
class SimplePredictor:
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "sales" in df.columns:
            ...
            for window in [3, 7, 14, 21, 30]:
                ...
                df[f"sales_rolling_{window}_median"] = (
                    df["sales"].rolling(window).median()
                )
    ...
    def predict(
        ...
        try:
            ...
            if len(historical_df) > 0 and "sales" in historical_df.columns:
                ...
                for window in [3, 7, 14, 21, 30]:
                    if len(recent_sales) >= window:
                        ...
                        future_df[f"sales_rolling_{window}_median"] = np.median(
                            window_data
                        )
                    else:
            ...
            if (
                "store_id" in future_df.columns
                and future_df["store_id"].dtype == "object"
            ):
                # If we have encoders, use them
                if (
                    self.model_loader.encoders
                    and "store_id" in self.model_loader.encoders
                ):
                    try:
                        # Transform store_id
                    ...
                    if future_df["store_id"].str.contains("store_").any():
                        future_df["store_id"] = (
                            future_df["store_id"].str.extract("(\d+)").astype(int)
                        )
                    else:
                        future_df["store_id"] = 1
            ...
            if self.model_loader.feature_cols:
                ...
                store_cols = [
                    ...
                    if col.startswith("store_id_") and col != "store_id"
                ]
                feature_cols.extend(store_cols)
                available_features = [
                    col for col in feature_cols if col in future_df.columns
                ]
            ...
            if self.model_loader.scalers and "target" in self.model_loader.scalers:
                try:
                    predictions = (
                        self.model_loader.scalers["target"]
