        ...
        try:
            ...
            if os.path.exists(scalers_path):
                try:
                    ...
                    logger.warning(
                        f"Could not load scalers with joblib, trying pickle: {e}"
                    )
            ...
            if os.path.exists(encoders_path):
                try:
                    ...
                    logger.warning(
                        f"Could not load encoders with joblib, trying pickle: {e}"
                    )
            ...
            if os.path.exists(feature_cols_path):
                try:
                    ...
                    logger.warning(
                        f"Could not load feature_cols with joblib, trying pickle: {e}"
                    )
                    with open(feature_cols_path, "rb") as f:
                        self.feature_cols = pickle.load(f)
                    logger.info(
                        f"Loaded {len(self.feature_cols)} feature columns with pickle"
                    )
            ...
            if os.path.exists(models_dir):
                ...
                if os.path.exists(xgb_path):
                    try:
                        ...
                        logger.warning(
                            f"Could not load XGBoost with joblib, trying pickle: {e}"
                        )
                ...
                if os.path.exists(lgb_path):
                    try:
                        ...
                        logger.warning(
                            f"Could not load LightGBM with joblib, trying pickle: {e}"
                        )
                ...
                ensemble_path = os.path.join(
                    models_dir, "ensemble", "ensemble_model.pkl"
                )
                if os.path.exists(ensemble_path):
                    try:
                        ...
                        logger.warning(
                            f"Could not load Ensemble model with joblib: {e}"
                        )
