...
class ProductionModelLoader:
    ...
    def load_production_models(self) -> bool:
        ...
        try:
            ...
            for model_name in model_names:
                try:
                    ...
                    if production_version:
                        ...
                        logger.info(
                            f"Loaded {model_name} v{production_version.version} from production"
                        )
                    else:
            ...
            if self.loaded:
                ...
                logger.warning(
                    "No production models found. Using fallback to latest run."
                )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        info = {"loaded": self.loaded, "model_count": len(self.models), "models": {}}
        ...
        for model_name, version_info in self.model_versions.items():
            info["models"][model_name] = {
                ...
                "run_id": (
                    version_info["run_id"][:8] if version_info["run_id"] else "N/A"
                ),
            }
    ...
    def predict(self, X: pd.DataFrame, model_type: str = "ensemble") -> Any:
        ...
        if model_type not in self.models:
            raise ValueError(
                f"Model {model_type} not loaded. Available: {list(self.models.keys())}"
            )
