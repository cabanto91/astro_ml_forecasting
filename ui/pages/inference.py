...
with st.expander("📦 Model Management", expanded=not st.session_state.models_loaded):
    ...
    with col1:
        if not st.session_state.models_loaded:
            ...
            st.success(
                f"✅ Models loaded from run: {st.session_state.run_id[:8] if st.session_state.run_id else 'Unknown'}..."
            )
    ...
    with col2:
        if st.button("🔄 Load Latest Models", type="primary"):
            with st.spinner("Loading models from MLflow..."):
                ...
                if not run_id:
                    st.error(
                        "No trained models found in MLflow. Please train a model first."
                    )

                if run_id and st.session_state.model_loader.load_models_from_run(
                    run_id
                ):
                ...
                manual_data.append(
                    {
                        "date": date,
                        "store_id": (
                            store_id if store_id != "All Stores" else "store_001"
                        ),
                        "sales": sales,
                    }
                )
        ...
        if generate_clicked:
            with st.spinner("Generating predictions..."):
                results = st.session_state.predictor.predict(
                    input_data, model_type=model_type, forecast_days=forecast_days
                )
                ...
                if results["success"]:
                    ...
                    if (
                        model_type == "ensemble"
                        and len(results["model_predictions"]) > 1
                    ):
                        ...
                        for model_name, preds in results["model_predictions"].items():
                            if model_name != "ensemble":
                                comparison_data.append(
                                    {
                                        "Model": model_name,
                                        "Average Prediction": np.mean(
                                            preds[~historical_mask]
                                        ),
