...
class RealisticSalesDataGenerator:
    ...
    def get_seasonality_factor(
        self, date: pd.Timestamp, seasonality_type: str
    ) -> float:
    ...
    def generate_promotions(self) -> pd.DataFrame:
            ...
            for event_name, month, day, duration, discount in major_events:
                if event_name == "Black Friday":
                    ...
                    thursdays = pd.date_range(
                        november, november + timedelta(days=30), freq="W-THU"
                    )
                    ...
                    for d in range(duration):
                        ...
                        if promo_date <= self.end_date:
                            ...
                            promo_products = random.sample(
                                list(self.all_products.keys()), k=random.randint(5, 15)
                            )
        ...
        flash_dates = pd.date_range(
            self.start_date, self.end_date, periods=n_flash_sales
        )

        for date in flash_dates:
            promo_products = random.sample(
                list(self.all_products.keys()), k=random.randint(3, 8)
            )
            for product_id in promo_products:
                promotions.append(
                    {
    ...
    def generate_store_events(self) -> pd.DataFrame:
        ...
        for store_id, store_info in self.stores.items():
            ...
            closure_dates = pd.date_range(
                self.start_date, self.end_date, periods=n_closures
            )
            ...
            if random.random() < 0.3:  # 30% chance of renovation
                renovation_start = self.start_date + timedelta(
                    days=random.randint(100, 600)
                )
    ...
    def generate_sales_data(
        self, output_dir: str = "/tmp/sales_data"
    ) -> Dict[str, List[str]]:
        ...
        while current_date <= self.end_date:
            ...
            for store_id, store_info in self.stores.items():
                ...
                for product_id, product_info in self.all_products.items():
                    ...
                    size_factor = {"large": 1.0, "medium": 0.7, "small": 0.5}[
                        store_info["size"]
                    ]
                    price_factor = 1.0 / (
                        1.0 + product_info["price"] / 100
                    ...
                    cost = (
                        quantity * product_info["price"] * (1 - product_info["margin"])
                    )
            ...
            if (
                daily_inventory_data and current_date.dayofweek == 6
            ):  # Weekly on Sundays
