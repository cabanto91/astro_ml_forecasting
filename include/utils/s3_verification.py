...
def verify_s3_artifacts(
    run_id: str, expected_artifacts: Optional[List[str]] = None
) -> Dict[str, any]:
    """
    ...
    try:
        ...
        if "Contents" in response:
            ...
            results["success"] = (
                len(s3_objects) > 0 and len(results["missing_artifacts"]) == 0
            )
...
def log_s3_verification_results(results: Dict[str, any]):
    ...
    if results["success"]:
        ...
        if results["missing_artifacts"]:
            logger.error(
                f"  - Missing artifacts: {', '.join(results['missing_artifacts'])}"
            )
