...
class DataValidator:
            ...
            if col in df.columns:
                ...
                if actual_type != expected_type:
                    try:
                        ...
                        errors.append(
                            f"Column {col}: Cannot convert {actual_type} to {expected_type}"
                        )
    ...
    def validate_prediction_data(
        self, df: pd.DataFrame, training_stats: Dict[str, Any]
    ) -> Tuple[bool, List[str]]:
        errors = []
    ...
    def generate_validation_report(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Schema validation
        is_valid, schema_errors = self.validate_schema(df)
        report["schema_validation"] = {"is_valid": is_valid, "errors": schema_errors}
