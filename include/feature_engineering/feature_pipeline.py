...
class FeatureEngineer:
    ...
    def create_date_features(
        ...
        if "is_holiday" in date_features:
            ...
            df["is_holiday"] = (
                df[date_col].apply(lambda x: x in us_holidays).astype(int)
            )
    ...
    def create_lag_features(
        ...
        if group_cols:
            for lag in lag_values:
                df[f"{target_col}_lag_{lag}"] = df.groupby(group_cols)[
                    target_col
                ].shift(lag)
        else:
            for lag in lag_values:
        ...
        if group_cols:
            ...
            for window in windows:
                for func in functions:
                    col_name = f"{target_col}_rolling_{window}_{func}"
                    df[col_name] = (
                        df[target_col].rolling(window, min_periods=1).agg(func)
                    )
    ...
    def select_features(
        self, df: pd.DataFrame, target_col: str, importance_threshold: float = 0.001
    ) -> List[str]:
    ...
    def create_advanced_features(
        ...
        for span in ewm_spans:
            if group_cols:
                df[f"{target_col}_ewm_{span}"] = df.groupby(group_cols)[
                    target_col
        ...
        for window in [7, 30]:
            ...
            df[f"{target_col}_ratio_to_{window}d_avg"] = df[target_col] / (
                rolling_mean + 1
            )
    ...
    def create_target_encoding(
        ...
        for col in categorical_cols:
            ...
            for cat in counts.index:
                n = counts[cat]
                smooth_mean[cat] = (n * mean_target[cat] + smoothing * global_mean) / (
                    n + smoothing
                )
        ...
        logger.info(
            f"Created target encoding for {len(categorical_cols)} categorical features"
        )
        return df
