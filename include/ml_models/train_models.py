...
class ModelTrainer:
    ...
    def preprocess_features(
        ...
        feature_cols = [
            col for col in train_df.columns if col not in exclude_cols + [target_col]
        ]
    ...
    def train_xgboost(
        ...
        if use_optuna:
            ...
            study.optimize(
                objective, n_trials=self.config["training"].get("optuna_trials", 50)
            )
        ...
        try:
            ...
            if len(regressor_cols) > 5:
                ...
                regressor_cols = sorted(
                    variances.keys(), key=lambda x: variances[x], reverse=True
                )[:5]
    ...
    def train_all_models(
        ...
        try:
            ...
            self.mlflow_manager.log_model(
                xgb_model, "xgboost", input_example=X_train.iloc[:5]
            )
            ...
            self.mlflow_manager.log_model(
                lgb_model, "lightgbm", input_example=X_train.iloc[:5]
            )
            ...
            if prophet_enabled:
                try:
                    ...
                    if (
                        hasattr(prophet_model, "extra_regressors")
                        and prophet_model.extra_regressors
                    ):
                        regressor_cols = [
                            col for col in prophet_model.extra_regressors.keys()
                        ]
                        for col in regressor_cols:
                    ...
                    self.mlflow_manager.log_metrics(
                        {f"prophet_{k}": v for k, v in prophet_metrics.items()}
                    )
                    ...
                    logger.warning(
                        f"Prophet training failed: {e}. Using weighted ensemble of XGBoost and LightGBM."
                    )
            ...
            self.mlflow_manager.log_model(
                ensemble_model, "ensemble", input_example=X_train.iloc[:5]
            )
            ...
            try:
                ...
                from utils.s3_verification import (
                    verify_s3_artifacts,
    ...
    def _generate_and_log_visualizations(
        self, results: Dict[str, Any], test_df: pd.DataFrame, target_col: str = "sales"
    ) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                saved_files = visualizer.create_comprehensive_report(
                    ...
                    actual_data=test_df,
                    feature_importance_dict=(
                        feature_importance_dict if feature_importance_dict else None
                    ),
                    save_dir=temp_dir,
                )
    ...
    def save_artifacts(self):
        ...
        if "lightgbm" in self.models:
            joblib.dump(
                self.models["lightgbm"], "/tmp/models/lightgbm/lightgbm_model.pkl"
            )

        if "ensemble" in self.models:
            joblib.dump(
                self.models["ensemble"], "/tmp/models/ensemble/ensemble_model.pkl"
            )
