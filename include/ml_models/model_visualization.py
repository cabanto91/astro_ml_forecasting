...
class ModelVisualizer:
    ...
    def create_predictions_comparison_chart(
        ...
        for model_name, pred_df in predictions_dict.items():
            ...
            if (
                "prediction_lower" in pred_df.columns
                and "prediction_upper" in pred_df.columns
            ):
                ax.fill_between(
                    pred_df[date_col],
    ...
    def create_residuals_analysis(
        ...
        box_colors = [
            self.colors.get(model.lower(), "#95A5A6") for model in residuals_data.keys()
        ]

        bp = ax1.boxplot(
            box_data, labels=list(residuals_data.keys()), patch_artist=True
        )
        ...
        for model_name in residuals_data.keys():
            if model_name in merged_data:
                ...
                resid_values = (
                    residuals.iloc[:min_len]
                    if hasattr(residuals, "iloc")
                    else residuals[:min_len]
                )
        ...
        resid_array = (
            first_residuals.values
            if hasattr(first_residuals, "values")
            else first_residuals
        )
    ...
    def create_comprehensive_report(
        ...
        self.create_metrics_comparison_chart(
            metrics_dict, save_path=os.path.join(save_dir, "metrics_comparison.png")
        )
        saved_files["metrics_comparison"] = os.path.join(
            save_dir, "metrics_comparison.png"
        )
        ...
        saved_files["predictions_comparison"] = os.path.join(
            save_dir, "predictions_comparison.png"
        )
        ...
        saved_files["residuals_analysis"] = os.path.join(
            save_dir, "residuals_analysis.png"
        )
        ...
        saved_files["error_distribution"] = os.path.join(
            save_dir, "error_distribution.png"
        )
        ...
        if feature_importance_dict:
            ...
            saved_files["feature_importance"] = os.path.join(
    ...
    def _create_summary_figure(
        self, metrics_dict: Dict[str, Dict[str, float]], save_dir: str
    ) -> None:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(save_dir, "model_comparison_summary.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def generate_model_comparison_report(
    mlflow_manager, run_id: str, test_data: pd.DataFrame
) -> Dict[str, str]:
    ...
    saved_files = visualizer.create_comprehensive_report(
        metrics_dict, predictions_dict, test_data
    )
